...
└── js/
    ├── config.js     # 상수 (WS_BASE)
    ├── utils.js      # 순수 헬퍼 (getInitial, avatarColor)
    ├── socket.js     # ChatSocket 클래스
    ├── renderer.js   # DOM 렌더링 (메시지, 시스템)
    └── app.js        # 진입점 — DOM refs, 이벤트, 화면 전환
```
...
### js/config.js
```js
WS_BASE  — WebSocket 서버 주소 (변경 시 이 파일만 수정)
```
...
### js/app.js
```
showChat(username)  — 로그인 화면 숨김, 채팅 화면 표시, socket.connect()
showLogin()         — socket.disconnect(), 채팅 화면 숨김, 초기화
...
## 주의사항
- WS 주소 변경 시 `js/config.js`의 `WS_BASE`만 수정.
